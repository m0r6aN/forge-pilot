...
import { resolveDomainKeyFromIdea } from '@/lib/domain/resolveDomainKey'
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (domainKey) {
      // Try to hydrate domain profile from omega-core registry.
      // Gracefully degrades to null if not available - teaser proceeds without domain context.
      hydratedDomainProfile = await hydrateDomainProfile(domainKey, schemaId)
      if (!hydratedDomainProfile) {
        structuredInfo('teaser.domain_profile.not_found', {
          domainKey,
          schemaId,
          sessionId,
        })
    ...
    if (typeof diagnostics.message === 'string' && diagnostics.message.includes('Missing required env var')) {
